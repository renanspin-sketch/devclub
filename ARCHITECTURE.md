...
- **Otimização de imagens**: as duas primeiras imagens do site (`src/assets/projects/*.jpg`, seção Projetos) usam `loading="lazy"` + `width`/`height` explícitos vindos do dado (`Project.image`), para o navegador reservar o espaço correto e não causar layout shift antes do carregamento. Ainda em JPEG puro — WebP/AVIF com fallback fica para quando o volume de imagens justificar o esforço de pipeline.
