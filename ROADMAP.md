...
- [ ] Home em scrollytelling de 6 capítulos (Boot/Build/Deploy/Level Up/Connect/Hire) + páginas "Nossos Alunos"/Blog/Newsletter — briefing detalhado do usuário em 2026-07-24, ver `CHANGELOG.md`. Sub-fases:
  ...
  - [x] **Fase B — Home em 6 capítulos**:
    ...
    - [x] Level Up ganhou efeito de scrollytelling com pin: a seção fica presa na tela enquanto a pilha de trilhas avança uma de cada vez, ganha check ao ser concluída e dá lugar à próxima — pedido do usuário em 2026-08-04, a partir de uma implementação de referência (HTML/CSS/JS puro, original, sem texto/código do site) que ele trouxe pra adaptar à stack React/Tailwind/Framer Motion do projeto. Mesma técnica de pin do Boot (`useScroll` na própria section + `useSpring`). Ver `CHANGELOG.md` pro bug de contraste real (não do axe-core) achado e corrigido durante a adaptação
  ...
  - [x] Menu principal expandido de 3 pra 8 itens (Home, Sobre, Formações, MBA, Blog, Newsletter, Nossos Alunos, Login) — pedido do usuário em 2026-08-04. Sobre/Formações/MBA/Login ganharam rotas e páginas placeholder novas (mesmo padrão de Nossos Alunos/Blog/Newsletter); conteúdo real de todas as quatro ficaria pra uma Fase F. Revisado no mesmo dia (ver linha abaixo) antes de qualquer conteúdo real entrar — a maioria dos itens virou link externo
  - [x] Menu revisado de novo, mesmo dia, com os destinos reais do usuário: `Formações` aponta pra seção de trilhas da própria Home (`/#level-up`, rola até lá sem recarregar); `MBA`, `Nossos Alunos` e `Login` viraram links externos de verdade (`mba.`, `stars.`, `aulas.devclub.com.br`) — as páginas placeholder que tinham sido criadas pra essas três (`Mba.tsx`, `Login.tsx`, `Formacoes.tsx`) foram removidas por ficarem órfãs, sem nenhum link apontando pra elas; `Blog`/`Newsletter` saíram do menu (as páginas e rotas continuam existindo); `Área do aluno` saiu do cabeçalho (redundante com o novo `Login`); CTA "Quero fazer parte" do cabeçalho (só o do cabeçalho, o do Capítulo 6/Hire continua sendo o botão de copiar e-mail) passou a apontar pro WhatsApp real do usuário. `Home` força reload de página em vez de navegação client-side, também a pedido do usuário. `Sobre` é a única página placeholder nova que sobrou — segue sem data de conteúdo real
  - [x] **Sobre** ganhou conteúdo real, baseado num print de referência trazido pelo usuário: badge "indicação", "Para quem é o DevClub?", 4 estatísticas (10k+ alunos formados, 95% empregabilidade, 4.9/5, +15k) e 3 blocos "para quem" (começar do zero / se especializar / evoluir com IA, essa última citando "MBA em Inteligência Artificial... reconhecimento pelo MEC"). Layout/paleta seguem os tokens já estabelecidos do site (`accent-green`, `accent-violet`, `Reveal`). **Isso reverte a linha "fora de escopo" abaixo** pros itens de estatística e MEC — perguntado diretamente se deveria adaptar (tirar os números/MEC, mantendo o layout) ou usar exatamente como no print, o usuário escolheu manter exatamente como no print, uma decisão informada e explícita, não uma omissão minha
- **Fora do escopo, de propósito** (alegações factuais que não são verdadeiras para este projeto, **exceto onde já usadas de propósito** — ver Sobre acima): certificação MEC, contadores de alunos/números de escala, garantia de reembolso de produto pago, depoimento em vídeo real
